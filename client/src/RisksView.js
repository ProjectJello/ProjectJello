...
class RisksView extends Component {
        ...
        {this.state.showRiskCreator ? (
          ...
          {this.props.RisksData.length ? (
            this.props.RisksData.map((item, index) => (
                <Risk key={item.id} RiskData={item} onRiskStatusChange={this.props.onRiskStatusChange.bind(this)} onRiskDescriptionChange={this.props.onRiskDescriptionChange.bind(this)} onRiskNameChange={this.props.onRiskNameChange.bind(this)} />
              ))
            ) : (
              <h2>Risk Free!</h2>
          )}
  ...
  onRiskDescriptionChange(riskId, newDescription) {
    this.props.onRiskDescriptionChange(riskId, newDescription);
  }

  onRiskNameChange(riskId, newName) {
    this.props.onRiskNameChange(riskId, newName);
  }
}
